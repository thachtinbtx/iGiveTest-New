...
function sendEmail($i_parameters) {
    ...
    // Load new PHPMailer
    require_once $DOCUMENT_ROOT . '/inc/phpmailer_new/src/Exception.php';
    require_once $DOCUMENT_ROOT . '/inc/phpmailer_new/src/PHPMailer.php';
    require_once $DOCUMENT_ROOT . '/inc/phpmailer_new/src/SMTP.php';
    ...
    try {
        ...
        // Recipients
        // Use configured From address if available, otherwise fallback to parameter or default
        $fromEmail = defined('SMTP_FROM_EMAIL') ? SMTP_FROM_EMAIL : (isset($i_parameters['from']) ? $i_parameters['from'] : 'noreply@example.com');
        $fromName = defined('SMTP_FROM_NAME') ? SMTP_FROM_NAME : '';
        
        $mail->setFrom($fromEmail, $fromName);
        $mail->addAddress($i_parameters['to']);
        
        if (isset($i_parameters['from'])) {
        ...
        $mail->AltBody = strip_tags($i_parameters['body']);

        return $mail->send();
    } catch (Exception $e) {
        // For debugging, you might want to log the error: error_log("Message could not be sent. Mailer Error: {$mail->ErrorInfo}");
        return false;
    }
}
...
global $g_db;
$i_recordcount = 0;
$i_tables = explode(',', $i_table);
$i_tables_escaped = array();
foreach($i_tables as $t) {
    $t = trim($t);
    // If table name contains space (alias) or backticks, assume it's already handled or too complex for auto-escaping
...
$i_table_escaped = implode(', ', $i_tables_escaped);

$i_rSet1 = $g_db->Execute("SELECT COUNT(*) FROM ".$i_table_escaped.(($i_where != '') ? " WHERE ".$i_where : ''));
if($i_rSet1) {
 if(!$i_rSet1->EOF)
...
break;
case CONFIG_CONST_iseditor: 
 return '<div class="editor-container"><script language=JavaScript type="text/javascript">var '.$i_name_nospecialchars.'Editor = new InnovaEditor("'.$i_name_nospecialchars.'Editor"); '.$i_name_nospecialchars.'Editor.width="100%"; '.$i_name_nospecialchars.'Editor.cmdAssetManager="modalDialogShow(\''.$srv_settings['url_root'].'editors/iseditor/assetmanager/assetmanager.php?lang='.$lngstr['language_long'].'\', 640, 465)"; '.$i_name_nospecialchars.'Editor.btnFlash=true; '.$i_name_nospecialchars.'Editor.btnMedia=true; '.$i_name_nospecialchars.'Editor.btnStyles=true; '.$i_name_nospecialchars.'Editor.css="shared/style.css"; '.$i_name_nospecialchars.'Editor.btnStrikethrough=true; '.$i_name_nospecialchars.'Editor.btnSuperscript=true; '.$i_name_nospecialchars.'Editor.btnSubscript=true; '.$i_name_nospecialchars.'Editor.btnLTR=true; '.$i_name_nospecialchars.'Editor.btnRTL=true; '.$i_name_nospecialchars.'Editor.btnClearAll=true; '.$i_name_nospecialchars.'Editor.btnCustomTag=true; '.$i_addon.' '.$i_name_nospecialchars.'Editor.RENDER("'.getJSComplaintText($i_value).'");</script><input type="hidden" name="'.$i_name.'" id="'.$i_name_nospecialchars.'"></div>';
break;
case CONFIG_CONST_iseditor2: 
 return '<div class="editor-container"><textarea name='.$i_name.' cols='.$i_cols.' rows='.$i_rows.' id='.$i_name_nospecialchars.'>'.convertTextValue($i_value).'</textarea><script language=JavaScript type="text/javascript">var '.$i_name_nospecialchars.'Editor = new InnovaEditor("'.$i_name_nospecialchars.'Editor"); '.$i_name_nospecialchars.'Editor.width="100%"; '.$i_name_nospecialchars.'Editor.cmdAssetManager="modalDialogShow(\''.$srv_settings['url_root'].'editors/iseditor2/assetmanager/assetmanager.php?lang='.$lngstr['language_long'].'\', 640, 465)"; '.$i_name_nospecialchars.'Editor.btnFlash=true; '.$i_name_nospecialchars.'Editor.btnMedia=true; '.$i_name_nospecialchars.'Editor.btnStyles=true; '.$i_name_nospecialchars.'Editor.css="shared/style.css"; '.$i_name_nospecialchars.'Editor.btnStrikethrough=true; '.$i_name_nospecialchars.'Editor.btnSuperscript=true; '.$i_name_nospecialchars.'Editor.btnSubscript=true; '.$i_name_nospecialchars.'Editor.btnLTR=true; '.$i_name_nospecialchars.'Editor.btnRTL=true; '.$i_name_nospecialchars.'Editor.btnClearAll=true; '.$i_name_nospecialchars.'Editor.btnPasteText=true; '.$i_addon.' '.$i_name_nospecialchars.'Editor.REPLACE("'.$i_name_nospecialchars.'");</script></div>';
break;
case CONFIG_CONST_tinymce:
    return '<textarea name="'.$i_name.'" class="tinymce-editor" style="width: 100%;">'.convertTextValue($i_value).'</textarea>';
    break;
}
}
...
echo '</td><td width=53 align=right></td><td width=3><img src="images/1x1.gif" width=3 height=1></td></tr></table>';
...
$i_rSet3 = $g_db->Execute("SELECT answerid, answer_text, answer_feedback, answer_correct FROM ".$srv_settings['table_prefix']."answers WHERE questionid=".$i_questionid." ORDER BY answerid");
if(!$i_rSet3) {
...
$i_answer_ids[$g_vars['page']['question'][$i_questionno]['answer_count']] = $i_rSet3->fields['answerid'];
...
$g_vars['page']['question'][$i_questionno]['answer'][$i_id + 1]['id'] = $i_answer_ids[$i_no];
...
while(!$i_rSet7->EOF) {
 ...
 $i_answers = array();
$i_rSet5 = $g_db->Execute("SELECT answerid, answer_text, answer_correct FROM ".$srv_settings['table_prefix']."answers WHERE questionid=".$i_rSet7->fields['questionid']." ORDER BY answerid");
if(!$i_rSet5) {
...
$i_answers_ids[$i_answerno] = $i_rSet5->fields['answerid'];
...
$i_rSet2 = $g_db->Execute("SELECT MAX(g.access_tests) as access_tests, MAX(g.access_testmanager) as access_testmanager, MAX(g.access_gradingsystems) as access_gradingsystems, MAX(g.access_emailtemplates) as access_emailtemplates, MAX(g.access_reporttemplates) as access_reporttemplates, MAX(g.access_reportsmanager) as access_reportsmanager, MAX(g.access_questionbank) as access_questionbank, MAX(g.access_subjects) as access_subjects, MAX(g.access_groups) as access_groups, MAX(g.access_users) as access_users, MAX(g.access_visitors) as access_visitors, MAX(g.access_config) as access_config FROM ".$srv_settings['table_prefix']."groups_users gu, `".$srv_settings['table_prefix']."groups` g WHERE gu.userid=".$G_SESSION['userid']." AND gu.groupid = g.groupid");
if($i_rSet2) {
 if(!$i_rSet2->EOF) {
...
case QUESTION_TYPE_FILLINTHEBLANK:
    $i_answer_text = trim($i_answers[0]);
    $i_rSet2 = $g_db->Execute("SELECT answer_text FROM ".$srv_settings['table_prefix']."answers WHERE questionid=".$i_questionid." AND answerid=1");
    if(!$i_rSet2) {
        showDBError('checkTestAnswer', 4);
    } else {
        if(!$i_rSet2->EOF) {
            // Normalize User Answer: Uppercase, Replace Đ/đ with D, Remove non-DS characters
            // \xc4\x91 is đ, \xc4\x90 is Đ in UTF-8
            $user_ans = str_replace(array("\xc4\x91", "\xc4\x90", 'đ', 'Đ'), 'D', $i_answer_text);
            $user_ans = mb_strtoupper($user_ans, 'UTF-8');
            $user_ans = preg_replace('/[^DS]/', '', $user_ans);
            
            // Normalize Correct Answer
            $correct_ans = str_replace(array("\xc4\x91", "\xc4\x90", 'đ', 'Đ'), 'D', $i_rSet2->fields['answer_text']);
            $correct_ans = mb_strtoupper($correct_ans, 'UTF-8');
            $correct_ans = preg_replace('/[^DS]/', '', $correct_ans);
            ...
            if($percent > 0) {
                $i_answer_correct = ($percent == 100) ? 2 : 1; // 2=Correct, 1=Partially Correct
                $i_answer_points = round($i_questionpoints * $percent / 100, 5);
            } else {
                $i_answer_correct = 0;
                $i_answer_points = 0;
            }
