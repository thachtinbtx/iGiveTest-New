...
$g_vars['page']['tables'][1]['columns'] = array(
    ...
    array('Time Away', 'Time Away', $srv_settings['table_prefix'].'results.result_time_away'),
);
$i_order_no = isset($_GET['order']) ? (int)$_GET['order'] : 2;
if($i_order_no>=count($g_vars['page']['tables'][1]['columns']))
 $i_order_no = -1;
if($i_order_no>=0) {
	$i_direction = "";
	if(isset($_GET['direction']) && $_GET['direction'] == 'DESC') $i_direction = "DESC";
	
    $i_order_addon = '&order='.$i_order_no.'&direction='.($i_direction ? "DESC" : "ASC");
    $i_sql_order_addon = ' ORDER BY '.$g_vars['page']['tables'][1]['columns'][$i_order_no][2].' '.($i_direction ? "DESC" : "ASC");
}
