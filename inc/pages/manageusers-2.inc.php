...
// Removed duplicate header - title is shown by _header.tpl.html
...
echo '<div class="bg-white rounded-2xl shadow-lg p-8 border border-slate-200 max-w-4xl mx-auto">';
echo '<form method=post action="users.php?userid='.(($f_userid > 0) ? $f_userid : '').'&action=edit">';
echo '<table class="w-full text-left border-collapse" cellpadding=5 cellspacing=1 border=0>';
$i_rowno = 0;
...
$strExpireDate = '<input name="user_expiredate" id="user_expiredate" value="'.$strExpireDateFormatted.'" class="px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" type=text size=20><a href="javascript:void(0);" title="'.$lngstr['calendar']['hint'].'"><img src="images/button-calendar.gif" alt="'.$lngstr['calendar']['hint'].'" class="calendar-icon ml-2 inline-block" onclick="return showCalendar(\'user_expiredate\', \''.$lngstr['language']['calendar']['date_format'].'\', \'24\', true);" onmouseover="this.className+=\' calendar-icon-hover\';" onmouseout="this.className = this.className.replace(/\s*calendar-icon-hover/ig, \'\');"></a>';
...
if(($G_SESSION['access_users'] != 1) && ($G_SESSION['access_users'] != 3)) {
	echo '<tr valign=top><td class="py-3 px-4 bg-indigo-50 text-indigo-900 font-semibold rounded-lg mt-4" colspan=2><a class="text-indigo-900 hover:text-indigo-700 no-underline block w-full" href="javascript:void(0)" onclick="javascript:toggleSection(\'div_users_groups\')">'.$lngstr['page_users']['section_groups'].'</a></td></tr>';
echo '<tr valign=top><td class=rowone colspan=2><div id=div_users_groups style="display:'.(isset($_COOKIE['div_users_groups']) && $_COOKIE['div_users_groups']=='Y' ? 'block' : 'none').'"><table class="w-full" cellpadding=5 cellspacing=1 border=0>'; 
	$i_groups_text = '';
$arrUserGroups = array();
...
echo '<tr valign=top><td class="py-3 px-4 bg-indigo-50 text-indigo-900 font-semibold rounded-lg mt-4" colspan=2><a class="text-indigo-900 hover:text-indigo-700 no-underline block w-full" href="javascript:void(0)" onclick="javascript:toggleSection(\'div_users_personal\')">'.$lngstr['page_users']['section_personal'].'</a></td></tr>';
echo '<tr valign=top><td class=rowone colspan=2><div id=div_users_personal style="display:'.(isset($_COOKIE['div_users_personal']) && $_COOKIE['div_users_personal']=='Y' ? 'block' : 'none').'"><table class="w-full" cellpadding=5 cellspacing=1 border=0>';
...
echo '</table></div></td></tr>';
echo '<tr valign=top><td class="py-3 px-4 bg-indigo-50 text-indigo-900 font-semibold rounded-lg mt-4" colspan=2><a class="text-indigo-900 hover:text-indigo-700 no-underline block w-full" href="javascript:void(0)" onclick="javascript:toggleSection(\'div_users_work\')">'.$lngstr['page_users']['section_work'].'</a></td></tr>';
echo '<tr valign=top><td class=rowone colspan=2><div id=div_users_work style="display:'.(isset($_COOKIE['div_users_work']) && $_COOKIE['div_users_work']=='Y' ? 'block' : 'none').'"><table class="w-full" cellpadding=5 cellspacing=1 border=0>';
...
echo '</table></div></td></tr>';
echo '<tr valign=top><td class="py-3 px-4 bg-indigo-50 text-indigo-900 font-semibold rounded-lg mt-4" colspan=2><a class="text-indigo-900 hover:text-indigo-700 no-underline block w-full" href="javascript:void(0)" onclick="javascript:toggleSection(\'div_users_additional\')">'.$lngstr['page_users']['section_additional'].'</a></td></tr>';
echo '<tr valign=top><td class=rowone colspan=2><div id=div_users_additional style="display:'.(isset($_COOKIE['div_users_additional']) && $_COOKIE['div_users_additional']=='Y' ? 'block' : 'none').'"><table class="w-full" cellpadding=5 cellspacing=1 border=0>';
...
echo '</table></div></td></tr>';
if(($G_SESSION['access_users'] != 1) && ($G_SESSION['access_users'] != 3)) {
	echo '<tr valign=top><td class="py-3 px-4 bg-indigo-50 text-indigo-900 font-semibold rounded-lg mt-4" colspan=2><a class="text-indigo-900 hover:text-indigo-700 no-underline block w-full" href="javascript:void(0)" onclick="javascript:toggleSection(\'div_users_notes\')">'.$lngstr['page_users']['section_notes'].'</a></td></tr>';
echo '<tr valign=top><td class=rowone colspan=2><div id=div_users_notes style="display:'.(isset($_COOKIE['div_users_notes']) && $_COOKIE['div_users_notes']=='Y' ? 'block' : 'none').'"><table class="w-full" cellpadding=5 cellspacing=1 border=0>';
writeTR2Fixed($lngstr['page_users']['notes'], getTextArea('user_notes', @$arrUserData['user_notes']));
echo '</table></div></td></tr>';
}	
echo '</table>'; 
echo '<div class="mt-8 flex justify-center gap-4">
      <input class="px-6 py-2 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 cursor-pointer transition-colors shadow-md border-none" type=submit name=bsubmit value=" '.$lngstr['button_update'].' "> 
      <input class="px-6 py-2 bg-white text-slate-700 font-semibold rounded-lg hover:bg-slate-50 cursor-pointer transition-colors shadow-sm border border-slate-300" type=submit name=bcancel value=" '.$lngstr['button_cancel'].' ">
      </div></form>';
echo '</div>';
