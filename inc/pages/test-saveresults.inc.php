...
if($G_SESSION['yt_attempts'] > 0) {
    $i_rSetAttempt = $g_db->Execute("SELECT * FROM ".$srv_settings['table_prefix']."tests_attempts WHERE testid=".$G_SESSION['testid']." AND userid=".$G_SESSION['userid']);
    if($i_rSetAttempt && $i_rSetAttempt->EOF) {
...
if($i_isok) {
    // Optimization: Fetch all answers in one go to avoid N+1 queries
    $results_array = $i_rSet3->GetArray();
    $question_ids = array();
    ...
    if (!empty($question_ids)) {
        $q_ids_str = implode(',', $question_ids);
        $ans_rs = $g_db->Execute("SELECT questionid, answer_text, answer_correct FROM ".$srv_settings['table_prefix']."answers WHERE questionid IN ($q_ids_str) ORDER BY answerid");
        if ($ans_rs) {
            while (!$ans_rs->EOF) {
                $qid = $ans_rs->fields['questionid'];
                if (!isset($all_answers[$qid])) $all_answers[$qid] = array();
                $all_answers[$qid][] = $ans_rs->fields;
                $ans_rs->MoveNext();
            }
            $ans_rs->Close();
        }
    ...
    foreach ($results_array as $row) {
        ...
        if (isset($all_answers[$row['questionid']])) {
            $i_answerno = 1;
            foreach ($all_answers[$row['questionid']] as $ans) {
                $i_answers[$i_answerno] = $ans['answer_text'];
                $i_answers_correct[$i_answerno] = $ans['answer_correct'];
                $i_answerno++;
            }
        }
        
        $i_detailed_correct = ($row['result_answer_iscorrect'] == IGT_ANSWER_IS_CORRECT);
        
        $i_result_detailed_3_items[$i_questionno] = $i_questionno.'. '.trim(getTruncatedHTML($row['question_text'], 0))."\n"; 
        $i_result_detailed_1_items[$i_questionno] = $i_result_detailed_3_items[$i_questionno];
        $i_result_detailed_1_items[$i_questionno] .= $lngstr['email_answer_iscorrect'].($row['result_answer_iscorrect']==IGT_ANSWER_IS_UNDEFINED ? $lngstr['label_undefined'] : ($row['result_answer_iscorrect']==IGT_ANSWER_IS_CORRECT ? $lngstr['label_yes'] : ($row['result_answer_iscorrect']==IGT_ANSWER_IS_PARTIALLYCORRECT ? $lngstr['label_partially'] : $lngstr['label_no'])))."\n";
        $i_result_detailed_1_items[$i_questionno] .= $lngstr['email_answer_points'].$row['result_answer_points']."\n";
        ...
        for($i = 1; $i < $i_answerno; $i++) {
            switch($row['question_type']) {
                ...
                case QUESTION_TYPE_TRUEFALSE:
                    $i_answers_given = (int)$row['result_answer_text'];
                ...
                case QUESTION_TYPE_MULTIPLEANSWER: 
                    $i_answers_given = explode(QUESTION_TYPE_MULTIPLEANSWER_BREAK, $row['result_answer_text']);
        ...
        $i_result_detailed_3_items[$i_questionno] .= $lngstr['email_answer_points'].$row['result_answer_points']."\n";
