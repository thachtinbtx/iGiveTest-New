...
if(!$i_rSet2) {
 ...
 // Pre-fetch all fixed questions to exclude them from random selection
 $fixed_question_ids = array();
 $test_structure = array();
 while(!$i_rSet2->EOF) {
     $test_structure[] = $i_rSet2->fields;
     if($i_rSet2->fields['question_type'] != QUESTION_TYPE_RANDOM) {
         $fixed_question_ids[] = $i_rSet2->fields['questionid'];
     }
     $i_rSet2->MoveNext();
 }
 $i_rSet2->Close();

 // Calculate random requirements per subject
 $random_reqs = array();
 foreach($test_structure as $slot) {
     if($slot['question_type'] == QUESTION_TYPE_RANDOM) {
         $sid = $slot['subjectid'];
         if(!isset($random_reqs[$sid])) $random_reqs[$sid] = 0;
         $random_reqs[$sid]++;
     }
 }

 // Fetch candidate random questions for each subject
 $random_candidates = array();
 foreach($random_reqs as $subjectid => $count) {
     $exclude_ids = implode(',', $fixed_question_ids);
     $exclude_clause = $exclude_ids ? " AND questionid NOT IN ($exclude_ids)" : "";
     
     // Optimized: Use ORDER BY RAND() and LIMIT to avoid fetching all questions into memory
     $sql = "SELECT questionid, question_points FROM ".$srv_settings['table_prefix']."questions WHERE subjectid=".$subjectid." AND question_type NOT IN (".QUESTION_TYPE_RANDOM.")".$exclude_clause." ORDER BY RAND() LIMIT ".$count;
     $rs = $g_db->Execute($sql);
     if ($rs) {
         $candidates = array();
         while(!$rs->EOF) {
             $candidates[] = $rs->fields;
             $rs->MoveNext();
         }
         $rs->Close();
         
         $random_candidates[$subjectid] = $candidates;
     }
 }

 // Assign questions to slots
 foreach($test_structure as $slot) {
     if($slot['question_type'] != QUESTION_TYPE_RANDOM) {
         $yt_questionids[$i_questioncounter + 1] = $slot['questionid'];
         $yt_pointsmax += $slot['question_points'];
         $i_questioncounter++;
     } else {
         $sid = $slot['subjectid'];
         if(isset($random_candidates[$sid]) && count($random_candidates[$sid]) > 0) {
             $selected = array_pop($random_candidates[$sid]);
             $yt_questionids[$i_questioncounter + 1] = $selected['questionid'];
             $yt_pointsmax += $selected['question_points'];
             $i_questioncounter++;
             
             // Add to fixed list to prevent re-selection if multiple subjects overlap (unlikely but safe)
             $fixed_question_ids[] = $selected['questionid']; 
         } else {
