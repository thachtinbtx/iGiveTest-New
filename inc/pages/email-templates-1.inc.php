...
// Floating orbs
echo '<div class="fixed -top-32 -left-32 w-64 h-64 bg-gradient-radial from-pastel-lavender/15 to-transparent rounded-full blur-3xl floating-slow pointer-events-none"></div>';
echo '<div class="fixed top-1/2 -right-32 w-64 h-64 bg-gradient-radial from-pastel-mint/15 to-transparent rounded-full blur-3xl floating pointer-events-none"></div>';

// Modern header
echo '<div class="glass-panel-strong rounded-2xl shadow-neumorphic p-6 mb-6 relative z-10">';
echo '<div class="flex items-center gap-4">';
echo '<div class="bg-pastel-lavender/20 rounded-2xl p-3 shadow-neumorphic-sm">';
echo '<svg class="w-8 h-8 text-pastel-lavender" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>';
echo '</div>';
echo '<div>';
echo '<h2 class="text-3xl font-bold text-pastel-lavender tracking-tight">'.$lngstr['page_header_emailtemplates'].'</h2>';
echo '<p class="text-slate-600 mt-1">'.$lngstr['tooltip_etemplates'].'</p>';
echo '</div>';
echo '</div>';
echo '</div>';
...
echo '<form name=etemplatesForm class=iactive method=post>';
...
echo '<a href="email-templates.php?action=create" class="btn-neumorphic-primary px-4 py-2.5 flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg><span class="font-semibold">'.$lngstr['label_action_create_etemplate'].'</span></a>';
echo '<button type="button" onclick="f=document.etemplatesForm;if (confirm(\''.$lngstr['qst_delete_etemplates'].'\')) { f.action=\'email-templates.php?action=delete&confirmed=1\';f.submit();}" class="btn-neumorphic-danger px-4 py-2.5 flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg><span class="font-semibold">'.$lngstr['label_action_etemplates_delete'].'</span></button>';
echo '</div>';
...
if($i_limitcount > 0) {
	...
	for($i = $nStartPage; $i <= $nEndPage; $i++) {
		if($i != $i_pageno)
			echo '<a href="email-templates.php?pageno='.$i.$i_url_pages_addon.'" class="px-3 py-1 rounded-lg glass-card shadow-neumorphic-sm hover:shadow-neumorphic text-sm font-semibold text-slate-700 transition-all">'.$i.'</a>';
		else echo '<span class="px-3 py-1 rounded-lg bg-pastel-lavender/30 text-pastel-lavender font-bold text-sm">'.$i.'</span>';
	}
	echo '</div>';
	...
	echo '<div class="flex gap-1">';
	if($i_pageno > 1) {
		echo '<a href="email-templates.php?pageno=1'.$i_url_pages_addon.'" class="p-2 rounded-lg glass-card shadow-neumorphic-sm hover:shadow-neumorphic transition-all" title="'.$lngstr['button_first_page'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"></path></svg></a>';
		echo '<a href="email-templates.php?pageno='.max(($i_pageno-1), 1).$i_url_pages_addon.'" class="p-2 rounded-lg glass-card shadow-neumorphic-sm hover:shadow-neumorphic transition-all" title="'.$lngstr['button_prev_page'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg></a>';
	} else {
...
echo '<th class="p-3 w-20 text-right font-semibold">'.$lngstr["label_etemplates_hdr_etemplateid"].'</th>';
echo '<th class="p-3 w-1/3 font-semibold">'.$lngstr["label_etemplates_hdr_etemplate_name"].'</th>';
echo '<th class="p-3 font-semibold">'.$lngstr["label_etemplates_hdr_etemplate_description"].'</th>';
...
while(!$i_rSet1->EOF) {
 ...
 $is_system = $i_rSet1->fields["etemplateid"] <= SYSTEM_ETEMPLATES_MAX_INDEX;
echo '<tr class="hover:bg-slate-50 transition-colors duration-200 group">';
echo '<td class="p-3 text-center"><input id=cb_'.$i_pagewide_id.' type=checkbox name=box_etemplates[] value="'.$i_rSet1->fields["etemplateid"].'" onclick="toggleCB(this);" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"></td>';
echo '<td class="p-3 text-right font-mono text-xs text-slate-400">#'.$i_rSet1->fields["etemplateid"].'</td>';
echo '<td class="p-3 text-slate-700 font-medium text-sm">'.getTruncatedHTML($i_rSet1->fields["etemplate_name"]).'</td>';
echo '<td class="p-3 text-slate-600 text-sm">'.$i_rSet1->fields["etemplate_description"].'</td>';
...
echo '<a href="email-templates.php?etemplateid='.$i_rSet1->fields["etemplateid"].$i_order_addon.$i_url_limit_addon.'&action=edit" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors" title="'.$lngstr['label_action_etemplates_edit'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>';
if(!$is_system) {
	echo '<a href="email-templates.php?etemplateid='.$i_rSet1->fields["etemplateid"].$i_order_addon.$i_url_limit_addon.'&action=delete" onclick="return confirmMessage(this, \''.$lngstr['qst_delete_etemplate'].'\')" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="'.$lngstr['label_action_etemplate_delete'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></a>';
} else {
