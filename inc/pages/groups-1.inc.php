...
echo '</div>';
echo '<div>';
echo '<h2 class="text-3xl font-bold text-pastel-cadet tracking-tight">'.$lngstr['page_header_managegroups'].'</h2>';
echo '<p class="text-slate-600 mt-1">'.$lngstr['tooltip_groups'].'</p>';
echo '</div>';
echo '</div>';
echo '</div>';
...
echo '<form name=groupsForm class=iactive method=post>';
...
echo '<a href="groups.php?action=create" class="btn-neumorphic-primary px-4 py-2.5 flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg><span class="font-semibold">'.$lngstr['label_action_create_group'].'</span></a>';
echo '<button type="button" onclick="f=document.groupsForm;if (confirm(\''.$lngstr['qst_delete_groups'].'\')) { f.action=\'groups.php?action=delete&confirmed=1\';f.submit();}" class="btn-neumorphic-danger px-4 py-2.5 flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg><span class="font-semibold">'.$lngstr['label_action_groups_delete'].'</span></button>';
echo '</div>';
...
if($i_limitcount > 0) {
	...
	for($i = $nStartPage; $i <= $nEndPage; $i++) {
		if($i != $i_pageno)
			echo '<a href="groups.php?pageno='.$i.$i_url_pages_addon.'" class="px-3 py-1 rounded-lg glass-card shadow-neumorphic-sm hover:shadow-neumorphic text-sm font-semibold text-slate-700 transition-all">'.$i.'</a>';
		else echo '<span class="px-3 py-1 rounded-lg bg-pastel-cadet/30 text-pastel-cadet font-bold text-sm">'.$i.'</span>';
	}
	echo '</div>';
	...
	echo '<div class="flex gap-1">';
	if($i_pageno > 1) {
		echo '<a href="groups.php?pageno=1'.$i_url_pages_addon.'" class="p-2 rounded-lg glass-card shadow-neumorphic-sm hover:shadow-neumorphic transition-all" title="'.$lngstr['button_first_page'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"></path></svg></a>';
		echo '<a href="groups.php?pageno='.max(($i_pageno-1), 1).$i_url_pages_addon.'" class="p-2 rounded-lg glass-card shadow-neumorphic-sm hover:shadow-neumorphic transition-all" title="'.$lngstr['button_prev_page'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg></a>';
	} else {
...
echo '<th class="p-3 w-20 text-right font-semibold">'.$lngstr['label_managegroups_hdr_groupid'].'</th>';
echo '<th class="p-3 w-1/3 font-semibold">'.$lngstr['label_managegroups_hdr_group_name'].'</th>';
echo '<th class="p-3 font-semibold">'.$lngstr['label_managegroups_hdr_group_description'].'</th>';
...
while(!$i_rSet1->EOF) {
 ...
 $is_system = $i_rSet1->fields["groupid"] <= SYSTEM_GROUP_MAX_INDEX;
echo '<tr class="hover:bg-slate-50 transition-colors duration-200 group">';
echo '<td class="p-3 text-center"><input id=cb_'.$i_pagewide_id.' type=checkbox name=box_groups[] value="'.$i_rSet1->fields["groupid"].'" onclick="toggleCB(this);" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"></td>';
echo '<td class="p-3 text-right font-mono text-xs text-slate-400">#'.$i_rSet1->fields["groupid"].'</td>';
echo '<td class="p-3 text-slate-700 font-medium text-sm">'.getTruncatedHTML($i_rSet1->fields["group_name"]).'</td>';
echo '<td class="p-3 text-slate-600 text-sm">'.$i_rSet1->fields["group_description"].'</td>';
...
echo '<a href="groups.php?groupid='.$i_rSet1->fields["groupid"].$i_order_addon.$i_url_limit_addon.'&action=edit" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors" title="'.$lngstr['label_action_group_edit'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>';
if(!$is_system) {
	echo '<a href="groups.php?groupid='.$i_rSet1->fields["groupid"].$i_order_addon.$i_url_limit_addon.'&action=delete" onclick="return confirmMessage(this, \''.$lngstr['qst_delete_group'].'\')" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="'.$lngstr['label_action_group_delete'].'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></a>';
} else {
