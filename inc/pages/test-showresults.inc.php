...
// Fetch Time Away
$i_rSetAway = $g_db->Execute("SELECT result_time_away FROM ".$srv_settings['table_prefix']."results WHERE resultid=".$G_SESSION['resultid']);
$i_time_away = 0;
if($i_rSetAway && !$i_rSetAway->EOF) {
    $i_time_away = $i_rSetAway->fields['result_time_away'];
    $i_rSetAway->Close();
}
$g_vars['page']['time_away'] = getTimeFormatted($i_time_away);
$g_vars['page']['time_away_label'] = "Time Away: " . $g_vars['page']['time_away'];
