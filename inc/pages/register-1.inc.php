...
$f_birthday_year = (int)readPostVar('birthday_year');
$f_birthday_month = (int)readPostVar('birthday_month');
$f_birthday_day = (int)readPostVar('birthday_day');
if(!$f_birthday_year) $f_birthday_year = date('Y') - 20; // Default to 20 years ago
if(!$f_birthday_month) $f_birthday_month = 1;
if(!$f_birthday_day) $f_birthday_day = 1;

$g_vars['page']['items']['birthday'] = @getInputField(getConfigItem(CONFIG_reg_birthday), $lngstr['label_birthday'], getCalendarEx('birthday', $f_birthday_year, $f_birthday_month, $f_birthday_day, NULL, NULL, date('Y') - 100, date('Y')));
...
function getInputField($i_showsettings, $i_caption, $i_value) {
    if($i_showsettings >= CONFIG_CONST_show_donotrequire) {
        $required_mark = ($i_showsettings >= CONFIG_CONST_show_autogenerate ? '<span class="text-red-500 ml-1">*</span>' : '');
        
        // Inject Tailwind classes into inputs
        $i_value = str_replace('class=inp', 'class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"', $i_value);
        $i_value = str_replace('class=slt', 'class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"', $i_value);
        $i_value = str_replace('<textarea', '<textarea class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"', $i_value);
        
        // Fix for calendar selects (they don't have standard classes usually)
        $i_value = str_replace('<select name=', '<select class="px-2 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all mr-1" name=', $i_value);

        return '<div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">'.$i_caption.$required_mark.'</label>
            '.$i_value.'
        </div>';
    }
    return '';
