...
*/
	...
	*/
	...
	function Connect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $forceNew = false) 
	{
		...
		if ($this->debug) $this->outp( $this->host.': '.$err);
		return $ret;
	}	
	...
	*/
	function StartTrans($errfn = 'ADODB_TransMonitor')
	{
		...
		if ($this->debug && $this->transCnt > 0) $this->outp("Bad Transaction: StartTrans called within BeginTrans");
		$this->BeginTrans();
		$this->transOff = 1;
	}
	...
	*/
	function CompleteTrans($autoComplete = true)
	{
		...
		if ($this->_transOK && $autoComplete) {
			if (!$this->CommitTrans()) {
				$this->_transOK = false;
				if ($this->debug) $this->outp("Smart Commit failed");
			} else
				if ($this->debug) $this->outp("Smart Commit occurred");
		} else {
			$this->_transOK = false;
			$this->RollbackTrans();
	...
	*/ 
	function Affected_Rows()
	{
		...
		if ($this->debug) $this->outp( '<p>Affected_Rows error</p>',false);
		return false;
	}
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	{	
		if (!is_numeric($secs2cache)) {
			...
			if ($sql === false) $this->outp( "Warning: \$sql missing from CacheSelectLimit()");
    ...
    */
	...
	global $ADODB_CACHE_DIR;
		...
		if (strlen($ADODB_CACHE_DIR) > 1 && !$sql) {
         ...
         if ($this->debug) {
            $this->outp( "CacheFlush: $dir<br><pre>\n". $this->_dirFlush($dir)."</pre>");
         } else {
            $this->_dirFlush($dir);
         }
         return;
      ...
      if (!@unlink($f)) {
         if ($this->debug) $this->outp( "CacheFlush: failed for $f");
      }
   }
   ...
   function _dirFlush($dir, $kill_top_level = false) 
   {
	...
	global $ADODB_CACHE_DIR;
		...
		if (strlen($ADODB_CACHE_DIR) > 1 && !$sql) {
			...
			if ($this->debug) {
				$this->outp( "CacheFlush: $cmd<br><pre>\n", system($cmd),"</pre>");
			} else {
				exec($cmd);
			}
			return;
	...
	* then we can scale to 12.8 million unique cached recordsets. Wow!
	...
	static $notSafeMode;
		...
		if ($createdir && $notSafeMode && !file_exists($dir)) {
			$oldu = umask(0);
			if (!mkdir($dir,0771)) 
				if ($this->debug) $this->outp( "Unable to mkdir $dir for $sql");
			umask($oldu);
		}
	...
	function &CacheExecute($secs2cache,$sql=false,$inputarr=false)
	{
		...
		// no cached rs found
			if ($this->debug) {
				if (get_magic_quotes_runtime() && !$this->memCache) {
					$this->outp("Please disable magic_quotes_runtime - it corrupts cache files :(");
				}
				if ($this->debug !== -1) $this->outp( " $md5file cache failure: $err (see sql below)");
			}
			...
			if ($rs && $this->memCache) {
				...
				if(!putmemCache($md5file, $rs, $this->memCacheHost, $this->memCachePort, $this->memCacheCompress, $this->debug)) {
					...
					if ($this->debug) $this->outp( " Cache write error");
				}
			} else
			if ($rs) {
				$eof = $rs->EOF;
		...
		// ok, set cached object found
			...
			if ($this->debug){ 
				...
				$this->outp( " $md5file reloaded, ttl=$ttl [ $s ]");
			}
		}
		return $rs;
	}
	...
	function& AutoExecute($table, $fields_values, $mode = 'INSERT', $where = FALSE, $forceUpdate=true, $magicq=false) 
	{
		...
		if ($where!==FALSE) $sql .= ' WHERE '.$where;
			...
			$this->outp('AutoExecute: Illegal mode=UPDATE with empty WHERE clause');
			return $false;
		}
		...
		case '1':
			...
			break;
		default:
			$this->outp("AutoExecute: Unknown mode=$mode");
			return $false;
		}
		$ret = false;
	...
	*/
	...
	global $_ADODB_ACTIVE_DBS;
		...
		if (!class_exists($class)) {
			$this->outp("Unknown class $class in GetActiveRcordsClass()");
			return $false;
		}
		$arr = array();
		foreach($rows as $row) {
	...
	function BeginTrans() 
	{
		if ($this->debug) $this->outp("BeginTrans: Transactions not supported for this driver");
		return false;
	}
   ...
   */
	...
	*/
	...
	global $ADODB_LASTDB;
		...
		//$this->outp(adodb_pr(get_declared_classes(),true));
		if (!file_exists($file)) $this->outp("Missing file: $file");
		else $this->outp("Syntax error in file: $file");
		return false;
	}
			...
			if (!$db) {
				...
				if ($errorfn) {
					...
					$errorfn('ADONewConnection', 'ADONewConnection', -998,
					 ...
					 $this->outp( "<p>ADONewConnection: Unable to load database driver '$db'</p>",false);
	...
	function &NewDataDictionary(&$conn,$drivername=false)
	{
		...
		if (!file_exists($path)) {
			$this->outp("Dictionary driver '$path' not available");
			return $false;
		}
		include_once($path);
