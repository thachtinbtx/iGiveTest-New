...
class ADODB_mysqli extends ADOConnection {
	...
	function _connect($argHostname = NULL, 
			  ...
			  $argDatabasename = NULL, $persist=false)
	  {
	    ...
	    if (is_null($this->_connectionID)) {
	      ...
	      if ($this->debug) 
				$this->outp("mysqli_init() failed : "  . $this->ErrorMsg());
	      return false;
	    }
		/*
		...
		*/
		...
		if ($ok) {
			...
			if ($this->debug) 
		  		$this->outp("Could't connect : "  . $this->ErrorMsg());
			return false;
	   }
	}
	...
	function _insertid()
	{
	  ...
	  if ($result == -1){
	      if ($this->debug) $this->outp("mysqli_insert_id() failed : "  . $this->ErrorMsg());
	  }
	  return $result;
	}
	...
	function _affectedrows()
	{
	  ...
	  if ($result == -1) {
	      if ($this->debug) $this->outp("mysqli_affected_rows() failed : "  . $this->ErrorMsg());
	  }
	  return $result;
	}
...
//	    $this->_connectionID = $this->mysqli_resolve_link($this->_connectionID);
	    ...
	    if ($this->_connectionID) {
			...
			if (!$result) {
		    	$this->outp("Select of database " . $dbName . " failed. " . $this->ErrorMsg());
			}
			return $result;		
		}
	    return false;	
	...
	global $ADODB_COUNTRECS;
		...
		if (!$mysql_res =  mysqli_query($this->_connectionID, $sql, ($ADODB_COUNTRECS) ? MYSQLI_STORE_RESULT : MYSQLI_USE_RESULT)) {
		    if ($this->debug) $this->outp("Query: " . $sql . " failed. " . $this->ErrorMsg());
		    return false;
		}
