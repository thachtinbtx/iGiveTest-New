...
function toggleSection(strID) {
	...
	// Check if using Tailwind CSS 'hidden' class
	if (ctlDIV.classList.contains('hidden')) {
		isHidden = true;
		ctlDIV.classList.remove('hidden');
	} else if (ctlDIV.style.display === 'none') {
		// Legacy inline style check
		isHidden = true;
		ctlDIV.style.display = 'block';
	} else {
		// Check if element has 'hidden' class OR style.display is not 'none'
		if (ctlDIV.style.display !== 'none' && !ctlDIV.classList.contains('hidden')) {
			// Currently visible - hide it
			if (ctlDIV.className.includes('Tailwind') || ctlDIV.className.includes('flex') || ctlDIV.className.includes('grid')) {
				ctlDIV.classList.add('hidden');
			} else {
	...
	document.cookie = strID + "=" + (isHidden ? 'Y' : 'N') + "; expires=Thu, 31 Dec 2035 23:59:59 GMT; path=/;";
	
	// Rotate icon if exists (for modern UI)
	var ctlIcon = document.getElementById('icon_' + strID);
	if (ctlIcon) {
		if (isHidden) {
			ctlIcon.classList.remove('rotate-180');
		} else {
			ctlIcon.classList.add('rotate-180');
		}
	}
}
