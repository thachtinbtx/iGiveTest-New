...
if(isset($_GET['questionid'])) {
 if(isset($_POST['bsubmit']) || isset($_POST['bsubmit2']) || isset($_POST['bsubmit3'])) {
 if($G_SESSION['access_questionbank'] > 1) {
 ...
 // Giao diện mặc định cho các loại câu hỏi khác
...
break;
case 'import': 
 $g_vars['page']['title'] = $lngstr['page_title_import_questions'].$lngstr['item_separator'].$g_vars['page']['title'];
if(isset($_POST['bsubmit'])) {
 if($G_SESSION['access_questionbank'] > 1) {
 include_once($DOCUMENT_PAGES."question-bank-import-submit.inc.php");
} else {
 gotoLocation('question-bank.php');
}
} else if(isset($_POST['bcancel'])) {
 ...
 include_once($DOCUMENT_PAGES."question-bank-import.inc.php");
}
break;
default:
